...
手机管家的 AI 通话反诈、跨场景反诈和 AI 换脸检测会在运行时拦截能力判断及相关组件调用。Activity 在系统启动流程中取消，Receiver、Service 和 Provider 的相关回调不会执行。电话里的国家反诈中心拦截服务只改它自己的支持判断和开关读写，不碰其他电话功能。

模块没有定时轮询。

## 恢复手机管家 AI 反诈

1.1.27 用于清理旧版本留下的组件禁用状态。确认升级并重启后可安装 1.1.28 或更新版本；后续版本不再包含这段迁移清理代码。停用或卸载模块并重启，手机管家 AI 反诈和电话里的国家反诈中心拦截服务都会按系统原本的状态工作。
